AlgorithmicScore {var <>resize, <>resize2, <>func, <>w, <>w2, <>w3, <>w4, <>w5, <>g, <>func2, extra;
  ...
  var <>express, <>tagWindow, <>tagBox, timerWindow;
  var <>numBox, <>ttimer;
...
//      penFunc = {arg symb, pos, height, colorPen= 0, fontSize=59;
  ...
  timer {arg newTime=10, clockAdj=1,winAdj = 0.8, winAdd = 20, rightWin=1, extraWin=false, stopWatch=false;
    var oldTime, t, winBoundsArr, left, top;
    var timerRect, timerRectArr, boxRect, timerFunction;
    ...
    if(ttimer.notNil, {
      if(ttimer.isPlaying, {ttimer.stop; ttimer.free});
    });
    ...
    timerRect = Rect(winBoundsArr[2]-((205*rightWin)*resize) + (winBoundsArr[0]-(64)),
      winBoundsArr[1]+(winAdd*resize), (250*resize)*winAdj, (180*resize)*winAdj);
    ...
    if(stopWatch, {
      timerFunction = {inf.do({
        oldTime = oldTime + 0.1;
        numBox.value_(oldTime.round(0.1));
        (0.1*clockAdj).yield; });};
    }, {
      "false".postln;
      timerFunction = {(newTime/0.1).do({
        oldTime = oldTime - 0.1;
  ...
  stopWatch {arg newTime=0, clockAdj=1,winAdj = 0.8, winAdd = 20, rightWin=1, extraWin=false;
    this.timer(newTime, clockAdj, winAdj, winAdd, rightWin, extraWin, true);
  }
