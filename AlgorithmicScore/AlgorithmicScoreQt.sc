AlgorithmicScoreQt {var <>resize, <>resize2, <>func, <>w, <>w2, <>w3, <>w4, <>w5, <>g, <>func2, extra, octava, <>down, n, taskOn, clickColor1, white1, clickColor2, white2, clickColor3, white3, clickColor4, white4, taskOn2, synth, synth2, trebleClef, bassClef, altoClef, <>movie, <>picture, x, y, <>staffArr, <>clefArr, clefFunc, <>storeArrayClef, <>pianoArray, drawLines, <>staffGap, windowType=1, <>spacing, <>noteAdjust, clearStaffArr,<>clock, text, text2, <>express, <>tagWindow, <>stopWatchWindow, <>stopWatchRoutine;
	...
	*screenSet {arg string="score", left=0, top=0, width=1366, height=768, firstResize=1.4266666666667;
	...
	findNotes {arg arrayNotes= [[0,[0,0,0,0,0,0]]], staffColor= \black;
		//arrayNotes: [pos, [staff, noteType, note, acc, oct, color]];
		var count, newArray;
		count = 0;
		newArray = [];
		...
		arrayNotes.size.do({
			...
			noteBlack = 106.asAscii.asString;
			noteWhite = 107.asAscii.asString;
			octava = "8va";
			quince = "15ma";
...
/*			quince = [49,53].asAscii.asString;*/
			...
			penFunc = {arg symb, pos, height, colorPen= 0, fontSize=59;
				...
				x = pos1+pos;
				y = ((noteAdjust*resize)+((5*height)+(down*resize)));
				...
				case
				{[noteBlack, noteWhite].includes(symb)} {
					Pen.font = Font( "NWCV15", fontSize*resize );
					Pen.color = colFunc;
					Pen.stringAtPoint( symb, Point( x, y));
				}
				{[octava, quince].includes(symb)} {
					Pen.font = Font( "Marion", fontSize*resize );
					Pen.color = colFunc;
					Pen.stringAtPoint( symb, Point( x, y));
				}
				{[noteBlack, noteWhite, octava, quince].includes(symb).not} {
			...
			{acc == 1} {newArray = newArray.add({
				penFunc.value(sharp, (pos*step)-(14*resize), noteHeight-(0.1*resize), color);
			});}
			{acc == 2} {newArray = newArray.add({
				penFunc.value(flat, (pos*step)-(14*resize), noteHeight-(0.1*resize), color);
			});}
			{acc == 3} {newArray = newArray.add({
				penFunc.value(nat, (pos*step)-(14*resize), noteHeight-(0.1*resize), color);});
			};

			case
			{oct == 1} {newArray = newArray.add({
				penFunc.value(octava, (pos*step)-(2.5*resize), (noteHeight+(4*resize)), color, 17);
			});}
			{oct == 2} {newArray = newArray.add({
				penFunc.value(octava, (pos*step)-(2.5*resize), (noteHeight+(13*resize)), color, 17);
			});}
			{oct == 3} {newArray = newArray.add({
				penFunc.value(quince, (pos*step)-(2.5*resize), (noteHeight+(4*resize)), color, 17);
			});}
			{oct == 4} {newArray = newArray.add({
				penFunc.value(quince, (pos*step)-(2.5*resize), (noteHeight+(13*resize)), color, 17);
			});};
	...
	findChromatic {arg array = [[0, [1, 0, "c#4", 0]], [0, [0, 0, "c#5", 1]], [1, [0, 0, "e6", 0]]];
		//array: [pos, [staff, noteType, noteName, color]]
		var count, newArray, pos, staff, noteType, noteName, color, white;
		var midiNote, noteClass, octave, oct, note, acc, col;
