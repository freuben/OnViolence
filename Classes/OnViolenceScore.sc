OnViolenceScore {var <>headOut, <>motorOut, <>motorVol, <>motorPan, s, basicPath, <>score, globalMIDI, globalTimes, start, end, bufferTimes, selectPitch, buffType, algoTimes, <>stepPedal, countPedalUp, countPedalDown,<>tempo=1, <>partials, <percglobalTimesGlob, partialTrig=0, document, <>bufferArr, <>node, <>rightWin, clock, <motorSound, movwin, <>src, <>imageScale, <>imageAdj, movieScale, movieWinScale, <>network, snapshot, <>pedalTime, pedalDownMistake, pedalUpMistake, netConnect, <>rrandArray, <>muteMovie=false, <>lowSen, <>highSen, <>leftSen, <>rightSen;
	
	*new {arg headOut=0, motorOut=0, motorVol=3, panVal=0, lowVal=0, highVal=127, leftVal=0, rightVal=127, src, scoreType=\macBookPro15, connect=false, hostcomputer="tremmac56150", port=57120;
	...
	initOnViolenceScore {arg outHead, outMotor, volMotor, panMotor, lowVal, highVal, leftVal, rightVal, srcID, scoreType, connect, hostcomputer, port;
		...
		basicPath =  Document.standardizePath("~/Library/Application Support/SuperCollider/Extensions/OnViolence/");
		...
		motorSound = OnViolenceMotor(motorOut, motorVol, motorPan, rectArr: ([ -40, (((score.w.bounds.height/2) - (252*score.resize/2+60)))] ++ ([ 21, 252 ]*score.resize)) );
	...
	funcPage {arg page=0; 
		{score.image((basicPath ++ "/Media/AlgoScore/score/onviolencescore" ++ page.asString ++ ".jpg"), imageScale, imageAdj);}.defer;
		stepPedal = page;
		this.pageFunc(page);
		if(network.notNil, {
	...
	pedalHigh {
		...
		{countPedalUp == 66} {score.tagClose; this.textOnly("FREE IMPROV", 63.5*(1/tempo), 41); }
		
		{countPedalUp == 67} {score.tagClose; this.funcPage(42);}
	...
	pedalDown {
	this.pedalLow; this.score.click4Note(0.05);	
	}
	
	pedalUp {
	this.pedalHigh; this.score.click4Note(0.05);	
	}
	...
	movie {arg whichMovie=1, rate=1, scale=1, page=43, displayNext=true;
		...
		partialTrig = 1;
		moviePath = basicPath ++ "/Media/AlgoScore/videos/battle" ++ whichMovie.asString ++ ".mov";
