OnViolenceScore {var <>headOut, <>motorOut, <>motorVol, <>motorPan, s, basicPath;
  ...
  *new {arg headOut=0, motorOut=0, motorVol=3, panVal=0, lowVal=0, highVal=127,
  ...
  initOnViolenceScore {arg outHead, outMotor, volMotor, panMotor, lowVal, highVal,
    ...
    pathString = "~/Library/Application Support/SuperCollider/Extensions/OnViolence/";
    basicPath =  Document.standardizePath(pathString);
    bufferArr = [Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/hihat.wav"),
      Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/woodblock.wav"),
      Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/ride.wav"),
      Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/snarerim.aif"),
      Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/conga.aif"),
      Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/kick.wav"),
      Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/snare1.wav"),
      Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/cowbell.aif"),
      Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/low_tom.wav"),
      Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/splash.wav"),
      Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/crash.aif"),
      Buffer.read(s, basicPath ++ "/Media/AlgoScore/samples/ride_open.wav")];
  ...
  displayScore {arg page=1, type=\macBookPro15;
    ...
    gofunc = {arg view, char, modifiers;
      ...
      {char == $p} {
        gowin = Window("",
          Rect((score.w.bounds.width/2),(score.w.bounds.height/2),120,150);
        ).front;
  ...
  funcChooseSample {arg sampleIndex = 0;
    ...
    bufferTimes = [ 0.025, 0.039185800697256, 0.061421079051404, 0.096273366492749,
      0.15090195807355, 0.21139096075449, 0.33134096229308, 0.51935443645014,
      0.81405277751885, 1.2759723958761, 2 ];
  ...
  funcPage {arg page=0;
    ...
    {score.image((basicPath ++ "/Media/AlgoScore/score/onviolencescore"
      ++ page.asString ++ ".jpg"), imageScale, imageAdj);}.defer;
    stepPedal = page;
    this.pageFunc(page);
    if(network.notNil, {
    ...
    if(motorSound.notNil, {
      if(page < 16, {
        {if(motorSound.window.visible.not, {
          motorSound.window.visible = true;
        });}.defer;
      }, {
        {if(motorSound.window.visible, {
          motorSound.window.visible = false;}
        );}.defer;
      });
    });
  }
  ...
  funcAlgoScore {arg sample=1,page=1, tempo1=1, repeat=1,pagePartial=0;
    ...
    bufferTimesMIDI = [ 0.025, 0.039185800697256, 0.061421079051404, 0.096273366492749,
      0.15090195807355, 0.21139096075449, 0.33134096229308, 0.51935443645014,
      0.81405277751885, 1.2759723958761, 2 ];
...

    {
      ...
      case
      {pagePartial == 0} {
        this.funcPage(page); partialTrig = 0;
      }
      {pagePartial == 1} {
        this.partialNotes(5.47*tempo2,page,"c 4".notemidi,"c 6".notemidi, "f")
      }
      {pagePartial == 2} {
        this.partialNotes(7.69*tempo2,51,"c 2".notemidi,"c 7".notemidi, "P")
      } //11.83
    }.fork;
  }
  ...
  funcPerc {arg percWhich=0, percsample = 4, perctempo = 1, adjEnd = 1, withPedal = 1;
    var step=0,percglobalTimes,percbufferTimes, percglobalTimes2, percglobalTimes3;
    var percindexesOf,percglobalTimes4,percbuffType, percalgoTimes,percstart,percend;
    ...
    percstart = [0.24318181818182,0.053636363636372,0.27500000000001,
      0.28136363636364,0.33454545454546, 0.12500000000001,0.026363636363641, 0, 0];
    percend = [2.825,2.3327272727273,2.7931818181818,4.1504545454545,
      2.0518181818182, 2.2613636363636, 10.541818181818, 3.06289, 7.7944];

    percbufferTimes = [ 0.025, 0.039185800697256, 0.061421079051404,
      0.096273366492749, 0.15090195807355, 0.21139096075449, 0.33134096229308,
      0.51935443645014, 0.81405277751885, 1.2759723958761, 2 ];
...

    {
      ...
      score.click1CloseTime(
        (percstart[percWhich]+percend[percWhich]*(1/perctempo)).postln
      );
    }.defer;
  ...
  pedalLow {
    if(partialTrig == 0, {
      ...
      if(pedalDownMistake.not, {
        ...
        {countPedalDown == 60} {score.tagClose;}
        {countPedalDown == 61} {score.tagClose;}
  ...
  pedalHigh {
    if(partialTrig == 0, {
      ...
      if(pedalUpMistake.not, {
        ...
        {countPedalUp == 57} {
          score.tagClose;
          this.funcAlgoScore(23,32,tempo,1);
        }
        {countPedalUp == 58} {
          score.tagClose;
          this.funcAlgoScore(24,33,tempo,1);
        }
        {countPedalUp == 59} {
          score.tagClose;
          this.funcAlgoScore(25,34,tempo,1,1);
          this.partialNum(18);
        }
        {countPedalUp == 60} {
          score.tagClose;
          this.funcAlgoScore(26,35,tempo,1);
        }
        {countPedalUp == 61} {
          score.tagClose;
          this.partialNotes(7.17*(1/tempo),36,"c 3".notemidi,"c 6".notemidi, "F");}
        {countPedalUp == 62} {
          score.tagClose;
          this.partialNotes(3.07*(1/tempo),37,"c 2".notemidi,"c 7".notemidi, "F");}
        {countPedalUp == 63} {
          score.tagClose;
          this.partialNotes(3.42*(1/tempo),38,"c 2".notemidi,"c 7".notemidi, "P");}
        {countPedalUp == 64} {
          score.tagClose;
          this.partialNotes(3.0*(1/tempo),39,"c 2".notemidi,"c 7".notemidi, "p", false);
          //this.funcPerc(8, 0, tempo, 1, 0);
        }
        {countPedalUp == 65} {
          score.tagClose; this.funcPage(40);
        }
        {countPedalUp == 66} {
          score.tagClose;
          this.textOnly("FREE IMPROV", 63.5*(1/tempo), 41);
        }
        ...
        {countPedalUp == 67} {
          ...
          this.funcPage(42);}
        {countPedalUp == 68} {
          this.movie(1,1/tempo,movieScale,43);
        }
        {countPedalUp == 69} {
          this.movie(2,1/tempo,movieScale,44);
        }
        {countPedalUp == 70} {
          this.textOnly("FREE IMPROV", 14.55*(1/tempo), 45);
        }
        {countPedalUp == 71} {
          this.funcPage(46);
        }
        {countPedalUp == 72} {
          this.movie(3,1/tempo,movieScale,47);
        }
        {countPedalUp == 73} {
          this.funcPage(48);
        }
        {countPedalUp == 74} {
          this.partialNotes(5.67*(1/tempo),49,"c 2".notemidi,"c 7".notemidi, "P");}
        {countPedalUp == 75} {
          this.movie(4,1/tempo,movieScale,50);
        }
        {countPedalUp == 76} {
          this.funcAlgoScore(27,34,tempo,1,2);
          this.partialNum(24); }
        {countPedalUp == 77} {
          this.funcAlgoScore(28,52,tempo,1)
        }
        {countPedalUp == 78} {
          this.funcAlgoScore(29,53,tempo,1)
        }
        {countPedalUp == 79} {
          this.funcPage(54);}
        {countPedalUp == 80} {
        ...
        {countPedalUp == 81} {
          this.textOnly("Don't play, solo electronics", 25*(1/tempo), 56, false, 60);
          {(26*(1/tempo)).yield;
            score.text("THE END", "Helvetica", 90, 1, 1);
            10.yield;
            score.textClose;
        }.fork(clock)};
  ...
  sensorX {arg val, leftLimit=0, rightLimit=127;
    var number1;

    number1 = val.linlin(leftLimit,rightLimit,0,127);
  ...
  sensorY {arg val, lowLimit=0, highLimit=127;
    var number2;

    number2 = val.linlin(lowLimit,highLimit,0,127);
  ...
  movie {arg whichMovie=1, rate=1, scale=1, page=43, displayNext=true;
    ...
    partialTrig = 1;
    moviePath = basicPath ++ "/Media/AlgoScore/videos/battle"
    ++ whichMovie.asString ++ ".mov";
    movieTimes = [42.706665039062, 33.934603174603, 14.552584157024,
      8.3358450927186];
...

    {
      score.clearWindow;
      0.1.yield;
      score.playMovie(moviePath, rate,	(1280-100)*(scale/1.2), (720-100)*(scale/1.2),
        50*(scale/1.2), mute: muteMovie);
  ...
  movieWin {arg whichMovie=1, scale=1;
    ...
    case
    {whichMovie == 1}{
      StaticText(movwin, 240@30*scale).string_("When you hear:").font_(
        Font("Helvetica", size: 25*scale)).stringColor_(Color.red);
      StaticText(movwin, 280@230*scale).string_(
        "BULLETS - play two notes a semitone a part on in each hand, each hand should not be further than a minor 3rd apart, all notes between c4 and c5.\r\rONE BULLET -> play two hands at the same time.\r\rREPEATED BULLETS -> alternate between two hands.\r\r\r\r";
      ).font_(Font("Helvetica", size: 16*scale)).stringColor_(Color.green);
      StaticText(movwin, 280@60*scale).string_(
        "BOMB -> play cluster with one hand bellow c2";
      ).font_(Font("Helvetica", size: 16*scale)).stringColor_(Color.yellow);
    }
    {[2,3].includes(whichMovie)}{
      StaticText(movwin, 240@30*scale).string_("When you hear:").font_(
        Font("Helvetica", size: 25*scale)).stringColor_(Color.red);
      StaticText(movwin, 280@60*scale).string_(
        "YOUR OWN BULLETS - play cluster with one hand between c4 and c5";
      ).font_(Font("Helvetica", size: 16*scale)).stringColor_(Color.blue);
      StaticText(movwin, 280@100*scale).string_(
        "BULLETS FROM OTHER COMBATANTS - play two notes a semitone apart alternating between hands, all above c6.\r\r";
      ).font_(Font("Helvetica", size: 16*scale)).stringColor_(Color.green);
      StaticText(movwin, 280@60*scale).string_(
        "BOMB -> play cluster with one hand bellow c2";
      ).font_(Font("Helvetica", size: 16*scale)).stringColor_(Color.yellow);
