...
OnViolence {var <>audioIn, s, <>ampIn, midiOut, d, <>amplitude, <>tempo=1;
  var <>countGlob=1, <>stepGlob=1, <>notes, <>pedalOffGlob, <>pedalOnGlob, <>piano;
  var osc, oscMax, <>sensorVal1=0, <>sensorVal2=63, <>grito1, <>grito2,<>grito3;
  var <>grito4, <>motor, <>metal, <>wagner1,<>wagner2, <>wagner3, <>wagner4;
  var <>buxtahude, <>parsifal, <>buxtasynths, <>metalOn, sensor, sensor1, sensor2;
  var bend1, bend2, basicPath, rightVal=46.628700256348, <>algoVersion, <>rrandArray;
  var <>differ, tempoStart=0,stepTempo=0,wagner1MIDITime, wagner1MIDINotes;
  var wagner1MIDIVel, wagner1MIDIEnd,wagner2MIDITime, wagner2MIDINotes, wagner2MIDIVel;
  var wagner2MIDIEnd,wagner3MIDITime, wagner3MIDINotes, wagner3MIDIVel, wagner3MIDIEnd;
  var wagner4MIDITime, wagner4MIDINotes, wagner4MIDIVel, wagner4MIDIEnd, document;
  var <>bufferArr, <>randBuffArr, <>volArr, <>panArr, <>network, <>master1, <>master2;
  var <>instr, <>partials, <>pan, <>instArr, condition, pnosoloRout, <>pnosoloVelGate=40;
  var <>pnosoloTranspAdj=1, <>pnosoloProb=0.4, <>pnosoloAdjVol=1, <>pnosoloVol=1;
  var <>chunks, <>chunksAdjDur=1.0, <>chunksTransp=1.0, <>chunksVolAdj=0.4;
  var <>chunksVolArr, <>chunksEqTrans=1, <>buxsynthAdjTime=1, <>lastSecVol=0.55;
  var <>ampPar=0.2, <>tempoTrack=1, <>trackingTempo=false, <>trackTempo;
  var <>silenceGritos=false, <>volArr1, <>volArr2;
  ...
  *new {arg audioIn = 0, volArr, panArr;
  ...
  initOnViolence {arg in, arrVol, arrPan;
    var thisPath, metalPath, screamPath, wagnerPath, buxtaPath, bufFunc, soundFileArr;
/*    if(NetAddr.langPort != 57120, {("SC Language port in not 57120, close other open applications and restart or change it in other computer to: " ++ NetAddr.langPort).warn;
    ...
    volArr = arrVol;
    volArr ?? {volArr = [1.0, 1.5, 0.5, 0.9, 1.0, 2.0, 0.2, 0.2, 0.2, 0.2, 1.0, 0.9];};
    //volMetal, volScream1, volScream2, volScream3, volScream4, volMotor, volWagner1,
    //volWagner2, volWagner3, volWagner4, volBuxta, volParsifal

    panArr = arrPan;
    panArr ?? {panArr = [0, -0.5, 0.5, 0, -1, 1, 0.25, -0.75, -0.25, 0.75, 0, 0];};
    //panMetal, panScream1, panScream2, panScream3, panScream4, panMotor, panWagner1,
    //panWagner2, panWagner3, panWagner4, panBuxta, panParsifal
    ...
    thisPath = "~/Library/Application Support/SuperCollider/Extensions/OnViolence";
    
    basicPath =  Document.standardizePath(thisPath);

    chunks = (basicPath ++ "/Data/chunksData.rtf").loadPath;
    
    basicPath = basicPath ++ "/Media/OnViolenceSamples";
    
    condition = Condition.new(true);

    instArr = [\chart1,\chart2,\wagner,\chart3,\chart4,\wagner,\chart5,\chart6,\wagner,
      \chart7,\chart8,\wagner,\chart9,\chart10,\wagner,\chart11,\chart12];

    rrandArray = Array.fill(124, {rrand(1,10)}); //select random samples;

    notes = [];

    metalPath = basicPath ++ "/metal/";
    screamPath = basicPath ++ "/scream/";
    wagnerPath = basicPath ++ "/wagner/";
    buxtaPath = basicPath ++ "/buxtahude/";

    bufFunc = {arg whichPath, soundFile;
      Buffer.read(s, whichPath ++ soundFile);
    };

    soundFileArr = ["metal_bang.scpv", "metal_bang2.scpv", "metal_bang3.scpv",
      "hitlerSpeech.aif", "screaming.scpv", "parsifal_kom.scpv", "parsifal_komkom.aif",
      "tristan_gate.aif", "tristan_shepardspipe.aif", "tristan_gatetrans1.aif",
      "tristan_gatetrans2.aif", "parsifal_transformation.aif", "parsifal_nun_achte_wohl.aif",
      "parsifal_heho.aif", "tristan_sol_ich_lauchen.aif", "tristan_preludeII.aif",
      "parsifal_ihr_schonen_kinder.aif", "tristan_lausch.aif", "parsifal_van_bade_kehrt.aif",
      "parsifal_preludeII.aif", "tristan_auf_as_tan.aif", "tristan_konig_merke_hei.aif",
      "tristan_verrater_ha.aif", "parsifal_vergeh.aif", "parsifal_weh_hoho.aif",
      "parsifal_met_diesem.aif", "buxtahude_left.scpv", "buxtahude_right.scpv",
      "parsifal_actIII_left.scpv", "parsifal_actIII_right.scpv"];

    s.makeBundle(nil, {

      soundFileArr.do{|item, index|
        case
        {(0..2).includes(index)} {bufferArr = bufferArr.add(bufFunc.value(metalPath, item));}
        {index == 3} {bufferArr = bufferArr.add(bufFunc.value(screamPath, item));}
        {index == 4} {bufferArr = bufferArr.add(
	        Buffer.read(s, screamPath ++ "screaming.scpv", 0, (9.37*2)*44100)
	    );}
        {(26..27).includes(index)} {bufferArr = bufferArr.add(bufFunc.value(buxtaPath, item));}
        {((5..25) ++ (28..29)).includes(index)} {
          bufferArr = bufferArr.add(bufFunc.value(wagnerPath, item));
        }
      };

      s.sync;

//      instr = NodeInst2(17, basicPath ++ "/singles chart/");
//      partials = NodePT(512, 12);
...
//      this.selectFiles(Array.fill(29, {rrand(1,10)}););
  ...
  selectFiles {arg arrayVersion;
    ...
    pathFolder = basicPath ++ "/randomSamples/";
  ...
  connect {arg hostcomputer="127.0.0.1", port;
    var ip;
    port ?? {port = 57120};
  ...
  startNodes {arg out1 = 0, out2 = 2;
...

    {
      ...
      0.25.yield;
      motor = OnViolenceMotor(0,1,panArr[5], rectArr: [1400, 20, 24, 288], playNode: false);
      0.25.yield;
  ...
  *initClass {
    ...
    StartUp.add{
      ...
      SynthDef.writeOnce(\pvplayMono, {arg out=0, recBuf=1, rate=1.0, amp=1.0, adjVol=1,
        thresh= 0.1, ratioUp = 1/3, ratioDown=1, pan=0, dec=0.9, gates=1, globamp=1.0;
        ...
        signal3 = Limiter.ar(signal2*adjVol, ratioUp, 0.01)*
        EnvGen.kr(Env.asr(0.01, 1.0, dec), gates);
        DetectSilence.ar(signal3, doneAction: 2);
        Out.ar(out, Pan2.ar(signal3, pan, amp*globamp));
      });

      SynthDef.writeOnce(\pvplayVoc, {arg out=0, recBuf=1, rate=1.0, amp=1.0, adjVol=1,
        pan=0, globamp=1.0;
        ...
        signal3 = Limiter.ar(signal2*adjVol) * EnvGen.kr(Env.asr(0.01, 1.0, 0.01),
          Amplitude.kr(signal));
        DetectSilence.ar(signal3, doneAction: 2);
        Out.ar(out, Pan2.ar(signal3, pan, amp*globamp));
      });

      SynthDef.writeOnce(\pvplayBet, {arg out=0, recBuf=1, rate=1.0, amp=1.0, adjVol=1,
        pan=0, globamp=1.0;
        var in, chain, signal, signal2, signal3, kernel, kernel2, freq, hasFreq, bufnum;
        bufnum = LocalBuf.new(2048, 1);
        chain = PV_PlayBuf(bufnum, recBuf, rate);
      ...
      SynthDef.writeOnce(\pvplayGrito, {arg out=0, amp=1.0, adjVol=0.9, thresh= 0.5,
        ratioDown=1, recBuf=4, rate=1, jefeBuf = 1, recBuf2 = 3, startPos = 0, osciOff=1,
        startOff=0, globamp=1.0, pan=0;
        ...
        signal2 = Limiter.ar(signal*adjVol) * EnvGen.kr(Env.asr(0.01, 1.0, (0.027*(osci*16))),
          Amplitude.kr(input))/2;
      ...
      SynthDef.writeOnce(\bang, {arg out=0, amp=0.5, recBuf=1, pan=0.0, rate=1, adjVol=1.0,
        lag=0.5, globamp=1.0;
      ...
      SynthDef.writeOnce(\playWagner, {arg rate=1, gate=1, amp = 1.0, start=0, atk=0.1,
        dec=0.2, trig=1, pan=0, buffer=0, out=0, globamp=1.0, globpan=0, loop=1;
        var signal, env;
      ...
      SynthDef.writeOnce(\pvplayBuxta2, { arg out=0, recBuf=1,amp=1,gate=1,rate=1,
        dec=2.0, pan=0.0, globamp=1.0, globpan=0;
      ...
      SynthDef.writeOnce(\stupidsquare, {arg pan = 0, out=0, freq=220, phase=0.4,
        amp=0.5, dur=1;
      ...
      SynthDef.writeOnce(\stupidsine, {arg pan = 0, out=0, freq=220, phase=0.4,
        amp=0.5, dur=1;
        var signal, signal2, env;
        signal = SinOsc.ar(freq, 0, 0.2);
      ...
      SynthDef.writeOnce(\stupidsine2, {arg pan = 0, out=0, freq=220, phase=0.4,
        amp=0.5, dur=1;
        var signal, signal2, env;
        signal = SinOsc.ar(freq, 0, 0.2);
      ...
      SynthDef.writeOnce(\stupidblip, {arg pan = 0, out=0, freq=220, phase=0.4,
        amp=0.5, dur=1;
        var signal, signal2, env;
      ...
      SynthDef.writeOnce(\stupidsaw, {arg pan = 0, out=0, freq=220, phase=0.4,
        amp=0.5,dur=1;
        var signal, signal2, env;
      ...
      SynthDef.writeOnce(\stupidsync, {arg pan = 0, out=0, freq=220, phase=0.4,
        amp=0.5,dur=1;
        var signal, signal2, env;
      ...
      SynthDef.writeOnce(\stupidlfsaw, {arg pan = 0, out=0, freq=220, phase=0.4,
        amp=0.5,dur=1;
        var signal, signal2, env;
      ...
      SynthDef.writeOnce(\stupidImpulse, {arg pan = 0, out=0, freq=220, phase=0.4,
        amp=0.5,dur=1;
        var signal, signal2, env;
